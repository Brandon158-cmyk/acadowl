...
const systemPrompt = `You are an AI tutoring assistant embedded in EduZambia, a Zambian school management platform. You are helping a student with their coursework.
...
- [ ] AI posts as `ai_tutor` participant in the thread: `{ role: 'ai_tutor', avatarIcon: '🤖', displayName: 'EduZambia Tutor' }`
...
- [ ] AI tutor messages visually distinct: subtle blue background, "EduZambia Tutor 🤖" attribution
...
- [ ] Platform admin view (at `platform.eduzambia.zm`): aggregate AI usage across all schools — most expensive features, most active schools
...
> **Goal:** Generate every Ministry of Education statistical return required from Zambian schools, pre-populated from EduZambia data. What used to take a week of manual counting takes one click.
...
The primary MoE annual return — enrolment by grade, gender, and school type; staffing qualifications; and infrastructure details. All data already exists in EduZambia — this aggregates it into the official format.
...
Physical infrastructure reporting — classrooms, toilets, water, electricity, library, boarding facilities. Much of this is manually entered (EduZambia doesn't track physical infrastructure) but boarding and transport data is auto-populated.
...
> **Audience:** Platform Super Admin at `platform.eduzambia.zm`
...
The platform admin's operational overview — all schools on the platform with health scores, usage metrics, and billing status. The tool that tells Anthropic/EduZambia which schools are thriving and which need support.
...
Platform-level announcements from EduZambia to all school admins — new feature releases, maintenance windows, policy updates.
...
- [ ] Platform admin creates announcements at `platform.eduzambia.zm/announcements`
...
Match mobile money bank statement entries against EduZambia payment records. A key monthly task for the bursar — resolving discrepancies between what Airtel/MTN deposited and what is recorded in the system.

#### Acceptance Criteria

- [ ] `getMobileMoneyReconciliationReport` query:
  - All mobile money payments recorded in EduZambia for the period
  - By provider: Airtel Money vs MTN MoMo
  - By context: fees vs pocket money vs transport
  - Unmatched payments: entries in `bankStatementImports` (Sprint 02 ISSUE-112) with no matching EduZambia record
  - Unmatched EduZambia records: payments in system with no corresponding bank statement entry
...
For schools that have been on EduZambia for multiple years — long-term trend analysis of enrolment, academic performance, fee collection, and at-risk rates.
...
_EduZambia Development Guide — Sprint 07 — College Mode + AI At-Risk Engine_
_Last updated: 2025 | Previous: Sprint 06 — Transport Module_
_This is the final sprint guide in the EduZambia development series._
