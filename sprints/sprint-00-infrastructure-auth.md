...
- [ ] Middleware correctly extracts `schoolSlug` from:
  - Production: `kabulonga.eduzambia.zm` → slug = `kabulonga`
...
- [ ] If no slug is found and path is not `/` or `/onboard`, middleware redirects to `www.eduzambia.zm`
- [ ] `src/lib/utils/schoolSlug.ts` utility function `getSchoolSlug(headers: ReadonlyHeaders): string | null` created
- [ ] Platform admin routes (`platform.eduzambia.zm`) handled separately — do not try to resolve a school
...
### ISSUE-011 · Configure Convex Auth with Multiple Providers
...
- [ ] After successful authentication, user document is created/updated:
  ...
  - `tokenIdentifier` matches the Convex Auth identity token
...
- [ ] If school slug not found, show generic EduZambia branding with "School not found" message
...
- [ ] Server-side helpers:
  - `src/lib/auth/getServerSession.ts` — wraps Convex Auth server utilities to get current user in Server Components
...
- [ ] Convex Auth's `profile()` callback updated to include `schoolId` in the JWT claims for server-side use
...
- [ ] Deactivate user: sets `isActive: false`, immediately invalidates sessions (Convex Auth handles this)
...
CONVEX_AUTH_PRIVATE_KEY=              # From Convex Auth setup
NEXT_PUBLIC_CONVEX_SITE_URL=          # e.g., https://eduzambia.zm (for auth redirect)

# ── AUTHENTICATION ──
# Convex Auth uses JWKS internally — run `npx convex auth configure`
AUTH_RESEND_KEY=                      # For email auth (optional — we primarily use SMS)

# ── SMS PROVIDERS ──
AIRTEL_SMS_API_KEY=                   # Airtel Zambia Developer API key
AIRTEL_SMS_SENDER_ID=                 # 'EduZambia' (must be registered with Airtel)
...
# ── APP ──
NEXT_PUBLIC_APP_URL=                  # e.g., https://eduzambia.zm
